...
export default function Component() {
  ...
  useEffect(() => {
    const data = postVoteFetcher.data as {
      success: boolean;
      message: string;
    } | null;
  ...
  useEffect(() => {
    const data = commentSubmitFetcher.data as {
      success: boolean;
      message: string;
    } | null;
    if (data?.success === true && data.message) {
  ...
  useEffect(() => {
    const data = commentVoteFetcher.data as {
      success: boolean;
      message: string;
    } | null;
    if (data?.success === true && data.message) {
  ...
  useEffect(() => {
    const votePostData = postVoteFetcher.data as {
      requiresUserValidation: boolean;
    } | null;
    const voteCommentData = commentVoteFetcher.data as {
      requiresUserValidation: boolean;
    } | null;
    const bookmarkData = bookmarkFetcher.data as {
      requiresUserValidation: boolean;
    } | null;
    const turnstileData = turnstileFetcher.data as {
      requiresUserValidation: boolean;
    } | null;
    const submitCommentData = commentSubmitFetcher.data as {
      requiresUserValidation: boolean;
    } | null;

    if (
      votePostData?.requiresUserValidation ||
      voteCommentData?.requiresUserValidation ||
      bookmarkData?.requiresUserValidation ||
      turnstileData?.requiresUserValidation ||
      submitCommentData?.requiresUserValidation
    ) {
          ...
          <div className="grid grid-cols-[auto_1fr] gap-2 mb-2 items-center">
            ...
            <div className="flex flex-wrap gap-y-3 my-2">
              {data.tags?.map((tag) => (
                <span
                  key={tag.tagName}
                  className="inline-block text-sm font-semibold text-gray-500 mr-1"
                >
