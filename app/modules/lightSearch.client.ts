...
export async function getOrCreateHandler(
  searchAssetURL: string,
  tagsAssetURL: string,
): Promise<LightSearchHandler> {
...
export class LightSearchHandler {
  ...
  async search(
    ...
    const searchResult = await this.searchByQueryAndTagsWithSort(
      query,
      orderby,
      page,
      tags,
  ...
  private async searchByQueryAndTagsWithSort(
    query: string,
    orderby: OrderBy,
    page: number = 1,
    tags: string[] = [],
    ...
    if (tags.length > 0) {
      if (tags.length === 1) {
        ...
        const tagConditions = tags
          .map((tag) => `'${tag}' IN (SELECT UNNEST(tagNames))`)
          .join(' AND ');
    ...
    const res = await conn.query(
      `SELECT * FROM search ${searchCondition} ${orderByClause} LIMIT ${pageSize} OFFSET ${offset}`,
    );
    await conn.close();
  ...
  async getTotalCount(query: string): Promise<number> {
    ...
    const countRes = await conn.query(
      `SELECT COUNT(*) as total FROM search WHERE postTitle LIKE '%${query}%' OR postContent LIKE '%${query}%'`,
    );
    await conn.close();
  ...
  private async getTagCounts(
    query: string,
    tags: string[],
  ): Promise<Array<{ tagName: string; count: number }>> {
    if (!this.db) {
      throw new Error('Database not initialized');
    }
    const conn = await this.db.connect();
    ...
    if (isQueryEmpty && isTagsEmpty) {
      ...
      if (tags.length === 1) {
        ...
        const tagSubqueries = tags
          .map((tag) => `SELECT postId FROM tags WHERE tagName = '${tag}'`)
          .join(' INTERSECT ');
  ...
  private getOrderByClause(orderby: OrderBy): string {
    switch (orderby) {
      case 'timeDesc':
        return 'ORDER BY postDateGmt DESC';
      case 'timeAsc':
        return 'ORDER BY postDateGmt ASC';
      case 'like':
        return 'ORDER BY countLikes DESC';
      default:
        return 'ORDER BY postDateGmt DESC';
    }
  }
