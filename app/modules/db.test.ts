...
import {
  ...
  getFeedComments,
  getFeedPosts,
  getLikedCommentsForTest,
  getNewestPostIdsForTest,
  getOldestCommentIdsForTest,
  getOldestPostIdsForTest,
  getRecentCommentIdsForTest,
  getRecentComments,
  getUnboundedLikesCommentIdsForTest,
  getUnboundedLikesPostIdsForTest,
  PostCardDataSchema,
} from './db.server';

const shouldSkipDbTests = process.env.SKIP_DB_TESTS === '1';

describe.skipIf(shouldSkipDbTests)(
  '記事ID23576の正しいデータを返すこと',
  async () => {
    test('記事ID23576の正しいデータを返すこと', async () => {
      ...
      expect(archiveDataEntry.tags).toContainEqual({
        tagName: 'やってはいけないこと',
        tagId: 5,
      });
      expect(archiveDataEntry.countLikes).toBeGreaterThan(30);
      expect(archiveDataEntry.countDislikes).toBeGreaterThan(5);
      expect(archiveDataEntry.postDateGmt).toEqual(
        new Date('2023-02-11T05:57:26.000Z'),
      );
      expect(archiveDataEntry.postContent).not.toBe('');
      expect(archiveDataEntry.similarPosts).toHaveLength(15);
      expect(archiveDataEntry.previousPost.postTitle).toBe(
        '無知識でアナルにローターを入れるべきでは無い',
      );
      expect(archiveDataEntry.nextPost.postTitle).toBe(
        '無能が消去法で大学を決めるべきではない',
      );
    });
  },
  ...
  async () => {
    ...
    describe('無期限いいね順の場合', async () => {
      test('無期限いいね順, 1ページ目', async () => {
        const unboundedLikesPostIds =
          await getUnboundedLikesPostIdsForTest(chunkSize);
        const pagingNumber = 1;
  ...
  async () => {
    ...
    describe('新着順の場合', async () => {
      test('新着順, 1ページ目', async () => {
        ...
        const pagingNumber = 1;
        const type = 'timeDesc';
        const feedComments = await getFeedComments(
          pagingNumber,
          type,
          chunkSize,
        );
        ...
        for (let i = 0; i < chunkSize; i++) {
          const comment = CommentShowCardDataSchema.parse(
            feedComments.result[i],
          );
      ...
      test('無期限いいね順, 1ページ目', async () => {
        const unboundedLikesCommentIds =
          await getUnboundedLikesCommentIdsForTest(chunkSize);
        const pagingNumber = 1;
        const type = 'unboundedLikes';
        const feedComments = await getFeedComments(
          pagingNumber,
