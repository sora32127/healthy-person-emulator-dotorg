...
import { eq, desc, asc, and, sql, count, inArray, lt, gt, like, not } from 'drizzle-orm';
  ...
  return {
    ...
    async getOldestPostIdsForTest(chunkSize: number) {
      const posts = await db
        .select({ postId: schema.dimPosts.postId })
        .from(schema.dimPosts)
        .orderBy(asc(schema.dimPosts.postDateGmt))
        .limit(chunkSize * 2);
    ...
    async getUnboundedLikesPostIdsForTest(chunkSize: number) {
      const posts = await db
        .select({ postId: schema.dimPosts.postId })
        .from(schema.dimPosts)
        .orderBy(desc(schema.dimPosts.countLikes), desc(schema.dimPosts.postDateGmt))
        .limit(chunkSize * 2);
      return posts.map((p) => p.postId);
    },
    ...
    async getRecentCommentIdsForTest(chunkSize = 12) {
      const comments = await db
        .select({ commentId: schema.dimComments.commentId })
        .from(schema.dimComments)
        .orderBy(desc(schema.dimComments.commentDateJst))
        .limit(chunkSize * 2);
      return comments.map((c) => c.commentId);
    },
    ...
    async getOldestCommentIdsForTest(chunkSize = 12) {
      const comments = await db
        .select({ commentId: schema.dimComments.commentId })
        .from(schema.dimComments)
        .orderBy(asc(schema.dimComments.commentDateJst))
        .limit(chunkSize * 2);
      return comments.map((c) => c.commentId);
    },
