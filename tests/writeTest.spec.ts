...
test.describe('投稿フォームのテスト', () => {
  ...
  test('最小限の要素を入力して投稿 * 結果悪', async ({ page }) => {
    ...
    // 5W1H+Thenの入力
    await page
      .getByLabel(/その状況の「主役」は誰ですか？/)
      .fill('テストユーザーが');
    await page.getByLabel(/いつ起こったことですか？/).fill('昨日');
    await page.getByLabel(/どこで起こったことですか？/).fill('公園で');
    await page
      .getByLabel(/なぜそのような行動をしたのですか？/)
      .fill('面白そうだったから');
    await page
      .getByLabel(/その主役は、何に対してはたらきかけましたか？/)
      .fill('友人に');
    await page
      .getByLabel(/その主役は、対象をどうしましたか？/)
      .fill('冗談を言った');
    await page
      .getByLabel(/行動の結果としてどうなりましたか？/)
      .fill('空気が悪くなった');

    // 健常行動ブレイクポイントの入力（最低1つ必要）
    await page
      .getByPlaceholder('友人の言動は冗談だという事に気が付く必要があった')
      .fill('相手の気持ちを考えていなかった');

    // どうすればよかったかの入力（最低1つ必要）
    await page
      .getByPlaceholder('冗談に対してただ笑うべきだった')
      .fill('黙っているべきだった');
  ...
  test('最大限の要素を入力して投稿 * 結果善', async ({ page }) => {
    ...
    await page
      .getByRole('textbox')
      .first()
      .fill('同居人は以前から料理を作ってくれていた');

    // なぜやってよかったのかの入力（最低1つ必要）
    await page
      .getByPlaceholder(/一般的に料理とは手間のかかる作業であり/)
      .fill('感謝の気持ちを伝えることで関係が深まる');
    await page
      .getByPlaceholder(/敬意はコミュニケーションに対して/)
      .fill('相手も嬉しそうだった');

    // やらなかったらどうなっていたかの入力（最低1つ必要）
    await page
      .getByPlaceholder(/相手がかけた手間に対して敬意をわないことは/)
      .fill('関係が希薄になっていた');
    await page
      .getByPlaceholder(/関係が改善されることはなく/)
      .fill('相手も不安になっていた');

    // 備考の入力
    await page
      .getByPlaceholder(/舌が過度に肥えてしまい/)
      .fill('その後も良好な関係が続いている');
  ...
  test('必須項目が未入力の場合はエラーが表示される', async ({ page }) => {
    await page.getByRole('button', { name: '投稿する' }).click();
    await expect(
      page.getByText('5W1H+Then状況説明>「誰が」は必須です'),
    ).toBeVisible();
  });

  test('反省/良かった点の入力が全て空の場合はエラーが表示される', async ({
    page,
  }) => {
    ...
    await expect(
      page.getByText(
        '「健常行動ブレイクポイント」もしくは「なぜやってよかったのか」は最低一つ入力してください',
      ),
    ).toBeVisible();
    await expect(
      page.getByText(
        '「どうすればよかったか」もしくは「やらなかったらどうなっていたか」は最低一つ入力してください',
      ),
    ).toBeVisible();
  });
