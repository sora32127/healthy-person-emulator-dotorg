...
test.describe('トップページ', () => {
  ...
  test('CommentShowCardが正しく表示されている', async ({ page }) => {
    await verifyComment(
      page.locator('.recent-comments').locator('> div').nth(0),
    );
  });
});
...
test.describe('記事検索', () => {
  ...
  */
  ...
  test('キーワードとタグで検索を行う', async ({ page }) => {
    ...
    await verifySearchResults(
      page,
      /検索結果 キーワード: Twitter タグ: やってはいけないこと/,
      10,
      2,
    );
  });
...
// 共通処理の関数化
async function verifyElementCount(
  page: Page,
  selector: string,
...
async function verifyElementMinCount(
  page: Page,
  selector: string,
  minCount: number,
) {
...
async function verifyElementTextMatch(
  locator: Locator,
  selector: string,
  pattern: RegExp,
) {
