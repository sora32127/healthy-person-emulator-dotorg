...
async function getAxeResults(page: Page) {
  ...
  const violationsLightMode =
    accessibilityScanResultsLightMode.violations.filter(
  ...
  if (violationsLightMode.length === 0) {
    console.log(
      chalk.green('No critical or serious violations found in light mode.'),
    );
  } else {
    console.log(
      chalk.red(
        `Found ${violationsLightMode.length} critical or serious violations in light mode:`,
      ),
    );
  ...
  if (violationsDarkMode.length === 0) {
    console.log(
      chalk.green('No critical or serious violations found in dark mode.'),
    );
  } else {
    console.log(
      chalk.red(
        `Found ${violationsDarkMode.length} critical or serious violations in dark mode:`,
      ),
    );
