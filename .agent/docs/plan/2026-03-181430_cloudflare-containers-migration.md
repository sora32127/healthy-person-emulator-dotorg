...
### Phase 5: 週次レポート移行（2-3日）

**作業内容:**
1. Container内の `report_weekly_summary.py` が動作することを確認
   - BigQuery認証（BIGQUERY_CREDENTIALS環境変数）
   - BQクエリ → Twitter投稿
2. `worker.ts` の `scheduled()` に週次レポートのCronハンドラーを追加
   - `0 12 * * 1`（毎週月曜12:00 UTC）
3. `wrangler deploy`
4. 旧Lambda側のReportWeeklySummaryのcronを無効化
5. 次の月曜にレポートが正常投稿されることを確認

---

### Phase 6: BigQuery ETL移行（3-4日）

**最もリスクが高い。最悪Lambda残置も選択肢。**

**作業内容:**
1. Worker側のD1→R2エクスポートロジックを実装
   - `app/modules/d1-export.server.ts` を新規作成
   - D1の各テーブルを1000行ずつクエリ → NDJSON → R2に保存
   - manifest.json を書き込み
2. Container内の `extract_and_load_to_bq.py` が動作することを確認
   - R2からNDJSONダウンロード → dltでBQにロード
   - BigQuery認証
3. `worker.ts` の `scheduled()` にETLのCronハンドラーを追加
   - `0 16 * * *`（毎日16:00 UTC）
4. 旧Lambda側のETL cronと**1週間並行稼働**（両方BQにロード、データ比較）
5. BQのテーブル件数・内容を旧ETLと比較して一致確認
6. 旧Lambda側のExtractAndLoadToBQのcronを無効化
