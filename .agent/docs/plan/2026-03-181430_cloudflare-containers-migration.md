...
**Step 2: 切り替え** 6. 旧Lambda側のCloudWatch Eventsを無効化（CreateOGImage の10分毎cron）7. 旧Lambda側のSNSトピックサブスクリプションを削除8. `AUTOMATION_DRY_RUN=false` に設定 → 新系統で本番投稿開始

**Step 3: 監視（1週間）** 9. 毎日Workers Logsを確認10. 各SNSで投稿が正常に出ているか確認11. social_post_jobs に `unknown` や `failed` がないか確認 12. 問題があれば `ENQUEUE_ENABLED=false` で即停止 → 旧Lambda復帰
...
1. **コンテナへの環境変数渡し（最大の課題）:**
   - `getEnv()` メソッド → Containersランタイムで呼ばれない（不動作）
   - `import { env } from "cloudflare:workers"` + `envVars` プロパティ → テスト用エンドポイントとの組み合わせで1101エラー発生
   - `get envVars()` getter + `this.env` → getterがランタイムに認識されず不動作
   - **最終解: Cloudflare Secrets Store + `startAndWaitForPorts()`** でインスタンス起動時に非同期で渡す。これが公式ドキュメント推奨の方法
   - BIGQUERY_CREDENTIALSはJSON値が長すぎてSecrets Storeに登録できず、Worker Secrets経由で対応

2. **Workers CPU制限（exceededCpu）:**
   - scheduledハンドラーでコンテナ起動を同期待ちすると30秒CPU制限に達する
   - **解決: `ctx.waitUntil()` でバックグラウンド実行** に変更

3. **`dim_posts` 投稿ID書き戻しエラー:**
   - `dim_posts` テーブルに `updated_at` カラムが存在しないのに `SET updated_at = ?` していた
   - **解決: `updated_at` の設定を削除**

4. **pendingジョブの再enqueue:**
   - `recoverStaleJobs()` が `queued` のみ対象で `pending` ジョブを拾わなかった
   - **解決: `pending` も再enqueue対象に追加**

5. **テスト用エンドポイントによるサイトダウン:**
   - `/__kick-automation` 等のテスト用パスを追加した際、1101エラーでサイト全体がダウン
   - **解決: テスト用パスを削除。今後はCron経由でのみテスト**

**SNS投稿 本番動作確認（2026-03-18 09:41 UTC）:**

| 記事  | Twitter                  | Bluesky                    | Misskey               |
| ----- | ------------------------ | -------------------------- | --------------------- |
| 48716 | ✅ `2034203569256071423` | ✅ `at://...3mhd73ek37p2j` | ✅ `ajzf1w87il2404iu` |
| 48717 | ✅ `2034203607197688108` | ✅ `at://...3mhd73iqtqn2i` | ✅ `ajzf238socjd031a` |
| 48718 | ✅ `2034203433201230282` | ✅ `at://...3mhd72rtihd2f` | ✅ `ajzf1lyy215802nn` |
| 48719 | ✅ `2034203541531713835` | ✅ `at://...3mhd7322bn32f` | ✅ `ajzf1rbqmkdh04yj` |

`dim_posts` への投稿ID書き戻しもD1直接実行で完了。

**作成・変更ファイル:**

- 新規: `container/tasks/report_legendary_article.py`, `app/modules/d1-export.server.ts`
- 変更: `worker.ts`, `wrangler.toml`, `container/main.py`, `container-worker.ts`, `app/modules/automation.server.ts`, `app/types/env.ts`

**監視項目（Phase 4 Step 3）:**

- 各SNSで投稿が正常に出ているか → ✅ 確認済み（4記事×3プラットフォーム成功）
- `social_post_jobs` に `unknown` / `failed` がないか → ✅ 全件 `sent` に更新済み
- 殿堂入りレポートが毎日12:00 UTCに動作するか → 要監視
- 週次レポートが月曜12:00 UTCに動作するか → 要監視
- BigQuery ETLが毎日16:00 UTCに動作するか → 要監視
...
- Cloudflare Containers のシークレット渡し方法（Secrets Store + startAndWaitForPorts）をfind-docsスキルのナレッジに追加すると良い
...
#### Phase 7 実施結果（2026-03-18）

**AWSリソース全削除完了。**

| リソース種別      | 削除件数 | 詳細                                                                                                                                                                                                                                                           |
| ----------------- | -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Secrets Manager   | 11件     | hpe-twitter-bot-tokens, hpe-bluesky-bot-tokens, MISSKEY_TOKEN, BIGQUERY_ACCESS_CREDENTIAL, HPE_INTERNAL_API, DLT_CONNECTION_PARAMS, SUPABASE_CONNECTION_SECRET, BASELIME_API_KEY, NEW_RELIC_CREDENTIALS, NEW_RELIC_LICENSE_KEY, my-toilet-blogs-r2-credentials |
| Lambda Functions  | 9件      | hpefunctions-prod-\* 全関数（CreateOGImage, PostTweet, PostBluesky, PostActivityPub, SaveSNSIdsToDB, ReportWeeklySummary, ReportLegendaryArticle, ExtractAndLoadToBQ, PickRandomArticle）                                                                      |
| CloudWatch Events | 5件      | hpefunctions-prod-\* 全ルール（ターゲット削除→ルール削除の順で実行）                                                                                                                                                                                           |
| SNS Topics        | 2件      | healthy-person-emulator-socialpost, healthy-person-emulator-socialpostIds                                                                                                                                                                                      |
| ECR Repository    | 1件      | serverless-hpefunctions-prod                                                                                                                                                                                                                                   |
| S3 Buckets        | 3件      | healthy-person-emulator-public-assets（OGP画像、R2移行済み）, hpefunctions-prod-serverlessdeploymentbucket-\* ×2                                                                                                                                               |

**削除後の確認:** Secrets=0, Lambdas=0, Events=0, SNS=0, ECR=0, S3(hpe)=0, サイト=200 OK

**プランからの変更点:**

- IAMロール・ポリシーの削除は未実施（Lambda削除で実質不要になるが、残存する可能性あり。後日確認）
- プラン外のシークレット（SUPABASE*CONNECTION_SECRET, BASELIME_API_KEY, NEW_RELIC*\*, my-toilet-blogs-r2-credentials）も不要と判断し一括削除
